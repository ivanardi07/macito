...
if (!function_exists('create_captcha')) {
	function create_captcha($data = '', $img_path = '', $img_url = '', $font_path = '')
	{
		...
		//=> exp microtime () 	: 0.75892100 1646714658 
		//=> explode() 			: array(0.75892100, 1646714658)
		//=> list()				: 
		...
		$colors 		= $data["colors"];
		$bg_color		= imagecolorallocate($im, $colors["background"][0], $colors["background"][1], $colors["background"][2]);
		...
		$grid_color		= imagecolorallocate($im, $colors["grid"][0], $colors["grid"][1], $colors["grid"][2]);
		...
		if ($use_font == FALSE) {
			...
			$font_size  = $font_size; //edited font for captcha size
			//$x = rand(0, $img_width / ($length / 1.5));
			$x = 10;
			$y = $font_size + 2;
		}
